...
class BackendConfiguration {
    ...
    @Bean("input_queue")
    fun inputQueue(): ConcurrentLinkedQueue<Message<Any>> {
    ...
    fun mqttInbound(@Autowired @Qualifier("mqttInputChannel") mqttInputChannel: MessageChannel): MessageProducer {
        val adapter = MqttPahoMessageDrivenChannelAdapter(broker, clientId + "_"+ UUID.randomUUID().toString(), mqttClientFactory(), "#")
    ...
    fun mqttOutbound(): MessageHandler {
        val messageHandler = MqttPahoMessageHandler(broker, clientId + "_"+ UUID.randomUUID().toString(), mqttClientFactory())
